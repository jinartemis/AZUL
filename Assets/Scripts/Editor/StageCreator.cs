        ...
        private static void CreateStageDatas()
        {
            ...
            foreach (var list in strList)
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    {
                        //★3
                        case 1: { _stageData.star3_score = int.Parse(list[i]); } break;
                        //★2
                        case 2: { _stageData.star2_score = int.Parse(list[i]); } break;
                        //★1
                        case 3: { _stageData.star1_score = int.Parse(list[i]); } break;
                        //ボーナス　レーン0
                        case 4: { _stageData.sheet[sheetNumber].lane[0].point = int.Parse(list[i]); } break;
                        //ボーナス　レーン1
                        case 5: { _stageData.sheet[sheetNumber].lane[1].point = int.Parse(list[i]); } break;
                        //ボーナス　レーン2
                        case 6: { _stageData.sheet[sheetNumber].lane[2].point = int.Parse(list[i]); } break;
                        //ボーナス　レーン3
                        case 7: { _stageData.sheet[sheetNumber].lane[3].point = int.Parse(list[i]); } break;
                    }
                    if (8 <= i && i < 24)
                    {
                        var laneNumber = (i - 8) / 4;    //(i < 13) ? 0 : (i < 17) ? 1 : (i < 21) ? 2 : 3;
                        var sheetTileNumber = (i - 8) % 4;
                        _stageData.sheet[sheetNumber].lane[laneNumber].tile[sheetTileNumber].type = (Define.TileType)(int.Parse(list[i]));
                    }
                    else if(25 <= i && i < 84)
                    {
                        if (list[i] == "") continue;
                        var poolNumber = (i - 25) / 6;
                        var poolTileNumber = (i - 25) % 6;
                        _stageData.pool[poolNumber].tile[poolTileNumber].type = (Define.TileType)(int.Parse(list[i]));
                    }
                    switch (i)
                    {
                        case 84: { data.mobText_jp = list[i]; } break;
                        case 85: { data.mobText_en = list[i]; } break;
                        case 86: { }break;  //BGM検索を後で追加する？
                    }
